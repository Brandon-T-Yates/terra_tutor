  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        if (user == null) return;
        String fileName =
            'profile_pictures/${user.uid}_${DateTime.now().millisecondsSinceEpoch}.png';
                ...
                if (passwordController.text.isNotEmpty) {
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(_dialogContext).showSnackBar(
                      SnackBar(
                          content:
                              Text('Re-authentication failed: ${e.message}')),
                    );
                  }
                } else {
                  ScaffoldMessenger.of(_dialogContext).showSnackBar(
      ...
      if (user != null) {
        ...
        if (!_canChangeEmail) {
          ...
          ScaffoldMessenger.of(_dialogContext).showSnackBar(
            const SnackBar(
              content: Text(
                  'You cannot change your email again until you log out and back in.'),
            ),
          );
          return;
        }
                                ...
                                onPressed: () async {
                                  if (newEmailController.text.isNotEmpty) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                            'Email update verification sent to new email address.'),
                                      ),
                                    );
                                  } else {
                                    ScaffoldMessenger.of(context).showSnackBar(
